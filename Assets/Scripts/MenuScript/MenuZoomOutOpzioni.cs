...
public class MenuZoomOutOpzioni : MonoBehaviour
{
    ...
    public OptionsMenu sceneLoader; // Assicurati che il tuo script di caricamento scena sia assegnato qui
        ...
        if (isZooming)
        {
            if (zoomIn)
            {
                ...
                if (Vector3.Distance(mainCamera.transform.position, zoomTarget.position) < 0.1f)
                {
                    ...
                    if (sceneLoader != null)
                    {
                        sceneLoader.Ritorna(); // Chiama il metodo di caricamento della scena
                    }
                }
            }
            else
    ...
    public void ZoomIn()
    {
        ...
        if (MainMenu.previousScene == "ModeSelectorMenu" || MainMenu.previousScene == "ScenaGlossario")
        {
            isZooming = false;
            sceneLoader.Ritorna();
        }
